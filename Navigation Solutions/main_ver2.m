...
%%% datasetDir = [currDir, '\dataset','\2022_04_25']; % Use this one to test
% datasetDir = [currDir, '\dataset','\2022_05_20'];
% datasetDir = [currDir, '\dataset','\2022_06_02'];
datasetDir = [currDir, '\dataset','\2022_09_02'];

% Journal Paper
datasetDir = [currDir, '\dataset','\2023_08_23']; % Subject Eudald
% datasetDir = [currDir, '\dataset','\2023_08_30']; % Subject Austin
% datasetDir = [currDir, '\dataset','\2023_09_07']; % Subject Chico


data_filename = 'exp200'; % without .mat %203
...
addpath([currDir, '\lib\Reconstruction lib']);

addpath(datasetDir);
load([data_filename '_VN.mat']);
% load([data_filename '_SVM_HD50.mat']);
% load([data_filename '_SVM_HD100.mat']);
% load([data_filename '_SVM_HD200.mat']);
% load([data_filename '_SVM_HD200.mat']);
% load([data_filename '_SVM_HD400.mat']);
load([data_filename '_SVM.mat']);

% loadedData = load([data_filename '_SVM.mat']); % For Journal Paper line of codes
% svm_results = loadedData.exp1000_SVM; % For Journal Paper line of codes

% load([data_filename '_SVM_HD400.mat']); % For Journal Paper line of codes datasets: 2022_09_02

% svm_window = 200; % if the HD dataset is used
svm_window = 1; % if the HD dataset is used

% u = u(:,svm_window+1-50:end-50); % Journal datasets

% u = u(:,svm_window+1:end);
% u = u(:,1+svm_window/2:end-svm_window/2);
...
if 0
    % c_energy = [0.5877 -1.1090 -0.0507];
    c_energy = [0.77224 -4.3265 7.4183];
    % c_energy = [0.696 -3.7831 6.3767];
    % coeff = [1.5,1.5,0.3];
    coeff = [1,1,1]*1;
    % coeff = [1,1,1];
    % c_energy_RMSE = [0.3643 -0.9711 2.6152];
    c_energy_RMSE = [0.3599 -1.2115 1.0053];
    sat_flag = zeros(3,length(u));
    load('GPR_Model.mat');
    %     sat_flag = abs(u(1:3,:))>= acc_thre;
    if 1
        sat_flag = (u(1:3,:))>= acc_thre(:,1)|(u(1:3,:))< acc_thre(:,2);


        period_seq = zeros(3,length(u_raw));
        peak_seq = zeros(3,length(u_raw));
        energy_seq = zeros(3,length(u_raw));
        reconstructed_signal = zeros(3,length(u_raw));
        reconstructed_signal_sigma = zeros(3,length(u_raw));

        for hh = 1:3
            beg_time = 0;
            end_time = 0;
            prev_flag = 0;
            for ll = 1:length(u_raw(8,:))
                %         reconstructed_signal(hh,ll) = 0;
                if sat_flag(hh,ll) == 1 && prev_flag == 0
                    beg_time = ll-1;
                    prev_flag = 1;
                end
                if sat_flag(hh,ll) == 0 && prev_flag == 1
                    end_time = ll;
                    period_seq(hh,ll) = u_raw(8,end_time) - u_raw(8,beg_time);
                    prev_flag = 0;
                    reconstructed_signal(hh,beg_time:end_time) = sign(sum(u_raw(hh,beg_time:end_time)))*2*predict(gprMdl_energy,period_seq(hh,ll)*1000)/(length(u_raw(hh,beg_time:end_time)))*(sawtooth(2*pi*((u_raw(8,beg_time:end_time)-u_raw(8,beg_time))/period_seq(hh,ll)),1/2)+1)/2;
                    % variance
                    reconstructed_signal_sigma(hh,beg_time:end_time) =  2*polyval(c_energy_RMSE,period_seq(hh,ll))/(period_seq(hh,ll)*1000)*g;
                end
            end
        end
        %     period_seq = period_seq+0.001;

        reconstructed_accelerometer = u(1:3,:) + reconstructed_signal;
        joint_period = [period_seq(1,~isnan(peak_seq(1,:))),period_seq(2,~isnan(peak_seq(2,:))),period_seq(3,~isnan(peak_seq(3,:)))]*1000;
        period_values = unique(round(joint_period,2));
        figure
        histogram(joint_period,length(period_values))
        alpha(0.2)
        ylim([0 140])
        legend('Histogram')
        ylabel('Number of data')
        ylabel('Saturation period')
        if if_recontructed
            u(1:3,:)  = reconstructed_accelerometer;
            data_filename = [data_filename '_Reconstructed'];
        end
    end
end
...
%
% R = diag(simdata.sigma_vel.^2); % measurement noise matrix
%
aB_l = zeros(3,1);
% aB_l = mean(v(cal-400+1:cal, 1:3))' - mean(v(cal-400+1:cal, 1:3))'/norm(mean(v(cal-400+1:cal, 1:3))')*g;
% gB_l = zeros(3,1);
% gB_l = mean(v(1:1000, 4:6))';
gB_l = mean(v(cal-400+1:cal, 4:6))';
% gB_l = mean(v(cal-1000+1:cal, 4:6))' + [0,0,-0.007]';

% gB_l = mean(v(cal-100+1:cal, 4:6))';
...
%     % SMV prediction
%     svm_results_temp(:) = svm_classifier.predict([v(i:i+1, 1:3)/g v(i:i+1, 4:6)]); 
%     
%     svm_results(end+1) = svm_results_temp(1);
...
% if ((zupt_l(i) == zupt)&&standaloneSVMdetector_switch~=1 ...
%             || standaloneSVMdetector_switch==1 && (svm_results(8,i) <= 9)) % SVM detection results
...
if (1)
    ...
    plot( est.t(cal+1:length(est.t)),  v(cal+1:length(est.t), 2));
    plot( est.t(cal+1:length(est.t)),  v(cal+1:length(est.t), 3));
    area( est.t(cal+1:length(est.t)),  (zupt_l(cal+1:length(est.t)))*20); grid; hold on;
    alpha(.3)
...
if (1)
    ...
    area( est.t(cal+1:length(est.t)),  (zupt_l(cal+1:length(est.t)))*5); grid; hold on;
    alpha(.3)
...
if (1)
    ...
    figure
    subplot(4,1,1);
    plot( true.t(cal+1:length(est.t)),  zupt_l(cal+1:length(est.t)), 'r'); hold on;
...
%     alpha(.05)
    ...
    figDspl = [figDspl, gcf];
    set( figDspl(end), 'Name', ttlMsg);
    saveas(gcf,[datasetDir '\' data_filename '_2d_path.fig'])
end
%% -------------------------------------------------------------------------
if (0)
    yL = {'(1)','(2)','(3)'};
...
if (0)
    ...
    %     area( est.t(cal+1:length(est.t)),  est.Northing_l(cal+1:length(est.t)) -3*sqrt(kf.diagP_l(1+6, cal+1:length(est.t))),'FaceColor','r');
    area( est.t(cal+1:length(est.t)),  (zupt_l(cal+1:length(est.t)))*0.4); grid; hold on;
    alpha(.3)
...
if (0)
    ...
    fill([est.t(cal+1:length(est.t)), fliplr(est.t(cal+1:length(est.t)))], inBetween, 'b--');      
    area( est.t(cal+1:length(est.t)),  (zupt_l(cal+1:length(est.t)))*0.1); grid; hold on;
    alpha(.3)    
...
if (1)
    ...
    ttlMsg = 'Log-Likelihood stadistics vs Classification results v2';
    title(ttlMsg);
    xlabel('Time, s');
    ylabel('Acceleration, m/s^2');
...
figure;
plot(est.t(1:end-W),log(abs(sum_IMU)));hold on
area(est.t(1:end), 10*(svm_results(8,1:length(est.t))<=9));
alpha(0.2)

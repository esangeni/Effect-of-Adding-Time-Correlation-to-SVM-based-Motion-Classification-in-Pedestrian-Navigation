function [zupt,total,total_x,total_y,total_z] = VAG_detector(u,v)
% -------------------------------------------------------------------------
...
total_x = sum(sum(u(1,:).^2))/sigma2_a+sum(sum(u(4,:).^2))/sigma2_g + sum(sum(v(1,:).^2))/(1e-7);
total_y = sum(sum(u(2,:).^2))/sigma2_a+sum(sum(u(5,:).^2))/sigma2_g + sum(sum(v(2,:).^2))/(1e-7);
total_z = sum(sum(u(3,:).^2))/sigma2_a+sum(sum(u(6,:).^2))/sigma2_g + sum(sum(v(3,:).^2))/(1e-7);

total = sum(sum(u(1:3,:).^2))/sigma2_a+sum(sum(u(4:6,:).^2))/sigma2_g + sum(sum(v(1:3,:).^2))/(1e-7);
total = total/c;

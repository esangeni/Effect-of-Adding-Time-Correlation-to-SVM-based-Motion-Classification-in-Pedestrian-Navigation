function [zupt,total,total_x,total_y,total_z] = SHOE_detector_Two(u)
% -------------------------------------------------------------------------
...
[r, c] = size(u);
W=simdata_Two.Window_size;
sigma2_a = (simdata_Two.sigma_a/simdata_Two.Ts)^2;
sigma2_g = (simdata_Two.sigma_g/simdata_Two.Ts)^2;
g = 9.796;
u_n = mean(u(1:3, :), 2);
...
if(total < simdata_Two.factor)
    zupt = ones(1, simdata_Two.Window_size);
else
    zupt = zeros(1, simdata_Two.Window_size);
end
end

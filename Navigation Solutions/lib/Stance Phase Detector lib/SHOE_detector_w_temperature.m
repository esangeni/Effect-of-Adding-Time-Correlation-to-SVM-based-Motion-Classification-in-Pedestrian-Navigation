function [zupt,total,total_x,total_y,total_z] = SHOE_detector_w_temperature(u,ax,ay,az,gx,gy,gz)
% -------------------------------------------------------------------------
...
total_x = sum(sum(u(1,:).^2))/(sigma2_a*ax)+sum(sum(u(4,:).^2))/(sigma2_g*gx);
total_y = sum(sum(u(2,:).^2))/(sigma2_a*ay)+sum(sum(u(5,:).^2))/(sigma2_g*gy);
total_z = sum(sum(u(3,:).^2))/(sigma2_a*az)+sum(sum(u(6,:).^2))/(sigma2_g*gz);

total = total_x+total_y+total_z;
% total = sum(sum(u(1:3,:).^2))/(sigma2_a*ax*ay*az)+sum(sum(u(4:6,:).^2))/(sigma2_g*gx*gy*gz);
total = total/c;

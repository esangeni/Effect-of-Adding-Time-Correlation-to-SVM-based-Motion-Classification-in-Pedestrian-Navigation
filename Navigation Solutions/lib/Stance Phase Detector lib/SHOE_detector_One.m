function [zupt,total,total_x,total_y,total_z] = SHOE_detector_One(u)
% -------------------------------------------------------------------------
...
[r, c] = size(u);
W=simdata_One.Window_size;
sigma2_a = (simdata_One.sigma_a/simdata_One.Ts)^2;
sigma2_g = (simdata_One.sigma_g/simdata_One.Ts)^2;
g = 9.796;
u_n = mean(u(1:3, :), 2);
...
if(total < simdata_One.factor)
    zupt = ones(1, simdata_One.Window_size);
else
    zupt = zeros(1, simdata_One.Window_size);
end
end

function [zupt,total,total_x,total_y,total_z] = SHOE_detector_Left(u)
% -------------------------------------------------------------------------
...
[r, c] = size(u);
W=simdata_Left.Window_size;
sigma2_a = (simdata_Left.sigma_a/simdata_Left.Ts)^2;
sigma2_g = (simdata_Left.sigma_g/simdata_Left.Ts)^2;
g = 9.796;
u_n = mean(u(1:3, :), 2);
...
if(total < simdata_Left.factor)
    zupt = ones(1, simdata_Left.Window_size);
else
    zupt = zeros(1, simdata_Left.Window_size);
end
end

function [zupt,LR,thre2] = adaptive_detector_w_downward_ultrasnoic(u, xi, dt, ZUPT, thre1, shock, shoe_height)
% -------------------------------------------------------------------------
...
%        shoe_height: readouts from downward-facing shoe-mounted ultrasonic
%        sensor
...
%     p = [0.0364 7.7276];  % According to the paper
    ...
    total = sum(sum(u(1:3,:).^2))/sigma2_a+sum(sum(u(4:6,:).^2))/sigma2_g + sum(shoe_height.^2)/sigma2_shoe;
    total = total/c;

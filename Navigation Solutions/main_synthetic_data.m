...
% datasetDir = [currDir, '\dataset','\2022_09_02'];   % Journal Paper Datasets
datasetDir = [currDir, '\dataset','\2022_11_22'];     % Synthetic IMU data walking model

data_filename = 'exp500_synthetic'; % without .mat %203
...
% load([data_filename '_SVM_opt.mat']);
% load([data_filename '_SVM_HD50.mat']);
% load([data_filename '_SVM_HD50_opt.mat']);
% load([data_filename '_SVM_HD100.mat']);
% load([data_filename '_SVM_HD100_opt.mat']);
% load([data_filename '_SVM_HD200.mat']);
% load([data_filename '_SVM_HD200_opt.mat']);
% load([data_filename '_SVM_HD400.mat']);
% load([data_filename '_SVM_HD400_opt.mat']);

svm_window = 0; % if the 0, 50,100,200,400 - HD dataset is used, 0 otherwise (HD datasets align 20*IMU, if 0 - align 15*IMU)
...
cal = 2; % Number of initial time steps for calibration

standaloneSVMdetector_switch = 10; % switch to use ZUPT event determined by svm
...
zupt = 10; % ZUPT switch 1 is on, 10 is off
...
load('exp500_ground_true_states.mat');
f_u=mean(v(2:cal,1));
f_v=mean(v(2:cal,2));
f_w=mean(v(2:cal,3));
...
% roll=groundTrueStates.euler_vect(1,1);
% pitch=groundTrueStates.euler_vect(2,1);
...
% gB_l = mean(v(cal-400+1:cal, 4:6))'*0;
